...
import { allCountries } from "@/utils/countryList.utils";
...
export function getCountryCodeFromName(name: string): string | undefined {
  ...
  return code?.toUpperCase(); // returns 'NG', 'US', etc.
}
...
export function isValidEmailone(email: string): boolean {
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
...
export function isValidNigerianNumber(phone: string): boolean {
  ...
  const regex = /^(?:\+234|234|0)(70|80|81|90|91|81|89|85|86|87|88)[0-9]{8}$/;
...
export function getError(e: any) {
  ...
  try {
    ...
    if (errorMap != null) {
      error = `${
        errorMap.message ?? "Unexpected error occurred, please try again later"
      }`;
    } else {
...
export function isLinkRestricted(path: string): boolean {
  ...
  if (path.includes("/kuvestore")) return false;
  if (path.includes("/popularstore")) return false;
  if (path.includes("/product-details")) return false;
  if (path.includes("/cart")) return false;
  if (path.includes("/wish-list")) return false;
  if (path.includes("/trendingproducts")) return false;
  if (path.includes("/cookie-policy")) return false;
  if (path.includes("/business/onboarding")) return false;
  if (path.includes("/storefront")) return false;

  return true;
}
export function formatStoreJoinMonthYear(dateString?: string | null): string {
...
export function toPostgresTimestamp(date: Date): string {
  ...
  return `${datePart} ${time}.${ms.padEnd(6, "0").replace("Z", "")}+00`;
}
